...
# read lens file
def lens_select(Rmin, Rmax, mirror=False):
    ...
    mra   = mdat['ra'][flg==1.0]
    mdec  = mdat['dec'][flg==1.0]
    mrsep = mdat['r'][flg==1.0]
    mpid  = mdat['id'][flg==1.0]
...
def run_pipe(Omegam=0.315, rmin=0.02, rmax=1.0, nbins=10, outputfile = 'pairs.dat', Rmin=0.3, Rmax=0.6, mirror=False, use_srcrand=False):
    ...
    lra, ldec, lred, lwgt = lens_select(Rmin=Rmin, Rmax=Rmax, mirror=mirror)

    # initializing arrays for signal compuations
    sumwls = np.zeros((len(rbins[:-1]),len(lra)))
    ...
    fout.write("# 0:rmin/2+rmax/2\t1:paircounts\t 2:lens_id\n")
    for i in range(len(rbins[:-1])):
        rrmin = rbins[i]
        rrmax = rbins[i+1]
        #Resp = sumwls_resp[i]*1.0/sumwls[i]

        for ll in range(len(lra)):
            fout.write("%le\t%le\t%d\n"%(rrmin/2.0+rrmax/2.0, sumwls[i][ll], ll))
    fout.write("#OK")
    fout.close()
...
if __name__ == "__main__":
    import sys
    mirr = 1==int(sys.argv[1])

    #def run_pipe(Omegam=0.315, rmin=0.02, rmax=1.0, nbins=10, outputfile = 'pairs.dat', Rmin=0.3, Rmax=0.6, mirror=False, use_srcrand=False):

    run_pipe(mirror=mirr, outputfile = 'random_pairs.dat', use_srcrand=True)
    #run_pipe(mirror=mirr)
